...
$scope.showTable = function(cb) { //this will update the displayed table
  ...
  $scope.clickedItTrans = function(i, j) {
		...
		function nemkell($rootScope, $scope, $timeout, $http, $interval, $compile) {
			...
			$rootScope.alertButton = function(thisCase, passData) {
      		...
      		$rootScope.takeItBack = function() {
				socketSend('moved', store.oopsStates[$scope.game._id], 'moved', function() {

					$scope.game.table = store.oopsStates[$scope.game._id].table
					$scope.game.wNext = store.oopsStates[$scope.game._id].wNext
					$scope.game.moves = store.oopsStates[$scope.game._id].moves
			...
			$scope.refreshTable = function() {
				...
				$http.get('/getTPollNum?t=' + $scope.game._id)
			...
			$scope.sendChat = function(chatLine) {
				...
				//console.log('sending chat:',chatLine,$scope.game._id)

				socketSend('boardChat', {
					chatLine: chatLine,
					gameNum: $scope.game._id
